...
public interface IDbService
{
    ...
    Task<bool> DeleteItemFromCartAsync(int cartId, int cartItemId);
    Task<bool> AddItemToCartAsync(int menuItemId, int cartId);
...
public class DbService : IDbService
{
    ...
    public async Task<bool> AddItemToCartAsync(int menuItemId, int cartId)
    {
    ...
    public async Task<bool> DeleteItemFromCartAsync(int cartId, int cartItemId)
    {
        Cart cart = await _dbContext.Carts.Where(c => c.Id == cartId).FirstOrDefaultAsync() ?? throw new ArgumentException("Cart not found");
